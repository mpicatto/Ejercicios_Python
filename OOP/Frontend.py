...
def search_comm():
    list1.delete(0,END)
    for row in db.search(title_text.get(), author_text.get(), year_num.get(), isbn_num.get()):
        list1.insert(END,row)

def add_entry():
    db.insert(title_text.get(), author_text.get(), year_num.get(), isbn_num.get())
    list1.delete(0,END)
...
def delete_entry():
    db.delete(selectedentry[0])
    list1.delete(0,END)
    for row in db.view():
        list1.insert(END,row)

def update_entry():
    db.update(selectedentry[0],title_text.get(), author_text.get(), year_num.get(), isbn_num.get())
    list1.delete(0,END)
    for row in db.search(title_text.get(), author_text.get(), year_num.get(), isbn_num.get()):
        list1.insert(END,row)
